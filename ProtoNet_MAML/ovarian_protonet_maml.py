...
from tensorflow.keras.layers import Dense, Conv1D, Flatten, GlobalAveragePooling1D
...
X_train = pd.read_csv(
    "/content/drive/My Drive/ICLR 2026/Datasets/Ovarian/ovarian_train_data.csv",
    index_col=0
)
y_train = pd.read_csv(
    "/content/drive/My Drive/ICLR 2026/Datasets/Ovarian/ovarian_train_labels.csv",
    index_col=0
)
y_train = y_train["file_label"]

X_test = pd.read_csv(
    "/content/drive/My Drive/ICLR 2026/Datasets/Ovarian/ovarian_test_data.csv",
    index_col=0
)
y_test = pd.read_csv(
    "/content/drive/My Drive/ICLR 2026/Datasets/Ovarian/ovarian_test_labels.csv",
    index_col=0
)
...
metrics_proto_cnn = computeMeasure(label_num, y_pred_proto_cnn, y_test.values)

print("ProtoNet_CNN metrics:")
...
y_pred_maml_cnn = predict_maml_cnn(
    maml_cnn_model, X_train_seq, y_train.values, X_test_seq
)
metrics_maml_cnn = computeMeasure(label_num, y_pred_maml_cnn, y_test.values)

print("MAML_CNN metrics:")
...
df_results = pd.DataFrame(rows, columns=["dataset", "model"] + metric_names)
csv_path = os.path.join(curr_path, "ovarian_meta_results.csv")
